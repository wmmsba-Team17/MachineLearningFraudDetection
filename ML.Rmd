...
We can also use head() function to get an idea of just how many missing variables we'll need to deal with later on in the process. As you can see by the output to the head function, it's a significant amount.

```{r}
head(train_transaction)
```

We can use the left join function to combine the train_identity and test_identity tables with the train_transaction and test_transaction tables, respectively. Once this is completed we can get 
rid of the four initial tables as they no longer serve a function for us.
...
Applying the same thought process we used previously when dealing with missing values, we can calculate the missing proportion with and visually see how many columns are missing values.

```{r}
(missing_rate <- colSums(is.na(full))/nrow(full)) 
hist(missing_rate,xlab='Percent of Values Missing',main='Full Data Missing Values')
```
